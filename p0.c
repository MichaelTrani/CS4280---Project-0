...
Output inclose a closed parenthsis for better readbility of outfile since 
...
#define BIGSTRING 10001  // Anticipating large input from keyboard

#define FILECOUNT 4  // Number of files for i/o
#define INFL 0  // Used for the infile on filename array
#define PREORD 1  // Used for the preorder outfile on filename array
#define INORD 2  // ... inorder outfile ...
#define POSTORD 3  // ... postorder outfile ...

void ioFileExtensions(char [], int);
void takeInString(char[]);

char filename[FILECOUNT][STDSTRING]; // Used to store i/o filenames


int main(int args, char* argv[]){

    char basefilename[STDSTRING];  //unmodified storage of filename
    
    if(args > 3) {  // Catches for invalid input from command line
    ...
    else if(args == 2)  // Anticipate filename argument

        strcpy(basefilename, argv[1]);  // Take cmd line filename for processing

    else if(args == 1)  // Keyboard input 

        strcpy(basefilename, "output"); // Apply default filename
    
    else {  // catch for anything unexpected

        printf("Unknown Error from args or argv.\n");  // Let user know what was fed to program
        printf("Number of arguments given: %d\n", args);  // Output arg count
        printf("Input received: ");
        
        int i = 0;
        for (i = 0; i < args; i++) 
            printf("%s", argv[i]);  // Output args for troubleshooting
    ...
    int j; 
    for (j = 0; j < 4; j++) {  // Filename processing
        strcpy(filename[j], basefilename); // Apply filename
        ioFileExtensions(filename[j], j);  // Apply appropiate extensions to filename
    }

     
    FILE *inf;  // Input file pointer

    if (args == 1) {  // Handling keyboard input

        takeInString(filename[INFL]);
    }
             
    inf = fopen(filename[0], "r"); // Open infile  
    if (inf == NULL) { // test for validity
        printf("Unable to open %s for input.\n", filename[INFL]);
        exit(1);
    }

  
    treeNode* root = treeSeed(inf);  // Build the binary search tree from the in file
    fclose(inf);  // Close the file


    preOrder(root, filename[PREORD]);
    inOrder(root, filename[INORD]);
    postOrder(root, filename[POSTORD]);
...
/* ioFileExtensions()
Selects appropate file extension based on id passed in
Takes length of filename to use as starting point for concatenation
Applies file extensions to passed char array in for loop.
Terminator is applied to completed filename
Called by: main()
*/
void ioFileExtensions(char filename[], int id) {
    
    #define EXTENSION_LENGTH 11  //This takes account for the longest file extension and terminating character
    char file_extension[FILECOUNT][EXTENSION_LENGTH] = {
        {".sp2020"},    // 0 - input file
        {".preorder"},  // 1 - output files
        {".inorder"},   // 2
        {".postorder"}  // 3
    };

    int i = strlen(filename);     // Obtain length of filename for starting point of the file extension.
    int j;                  // Starting point for the file_extension

    for (j = 0; file_extension[id][j] != '\0'; i++, j++) {  // Loop will halt when it reaches string terminator of file_extension
        
        filename[i] = file_extension[id][j];               // appends file_extension to the filename
    }
    filename[i] = '\0';  // add terminator
}


/* takeInString()
Takes filename passed in and creates FILE pointer.
The file is then opened and tested for validity
bigInput is declared 
It stores keyboard input a char array while searching for a simulated EOF (ctrl d)

Called by: main()
*/
void takeInString(char fileName[]) {

    FILE* strFile; // Output file pointer

        strFile = fopen(fileName, "w");  // Open infile 
    if (strFile == NULL) {  // Test for validity

        printf("Unable to open %s for input.\n", fileName);
        exit(1);
    }

    char bigInput[BIGSTRING];  // Char array to store user input from keyboard

    printf("Press ctrl+D up to *FOUR* times (sorry) after you have entered all your data.\n");
